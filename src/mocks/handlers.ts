...
export const handlers = [
  //로그인/가입
  //1. 로그인 시작화면
  rest.get("https://3.36.71.48/msw/auth", (req, res, ctx) => {
    return res(
      ctx.status(200),
      ctx.json({
        image: img8,
  ...
  //마이페이지
  //1. 유저 프로필 조회
  rest.get("https://3.36.71.48/msw/mypage", (req, res, ctx) => {
    //유저 토큰 확인
  ...
  rest.put<{ icon: string }>(
    "https://3.36.71.48/msw/mypage/edit",
    (req, res, ctx) => {
      //유저 토큰 확인
      const token = req.headers.get("authorization")?.split(" ")[1];
  ...
  //3. 스크랩 한 글
  rest.get("https://3.36.71.48/msw/mypage/scrap", (req, res, ctx) => {
    const token = req.headers.get("authorization")?.split(" ")[1];
    //토큰 확인
    if (token === "null") return res(ctx.status(401));
  ...
  //6. 업로드 한 글
  rest.get("https://3.36.71.48/msw/mypage/upload", (req, res, ctx) => {
    const token = req.headers.get("authorization")?.split(" ")[1];
    //토큰 확인
    if (token === "null") return res(ctx.status(401));
  ...
  //홈
  //1. 유저 프로필 받기
  rest.get("https://3.36.71.48/msw/home/profile", (req, res, ctx) => {
    //토근 확인
    const token = req.headers.get("authorization")?.split(" ")[1];
    if (token === "null") return res(ctx.status(401));
  ...
  //2. 스토리 정보 받기
  rest.get("https://3.36.71.48/msw/home/banner", (req, res, ctx) => {
  ...
  //3. 이벤트 정보 받기
  rest.get("https://3.36.71.48/msw/home/event", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
    return res(
      ctx.status(200),
      ctx.json([
  ...
  //4. 추천 포스트
  rest.get("https://3.36.71.48/msw/home/recommend", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
    return res(
      ctx.status(200),
      ctx.json([
  ...
  rest.post<{
    ...
    z: number; //고도
    date: Date; //작성 날짜
  }>("https://3.36.71.48/msw/arpost/create", (req, res, ctx) => {
    //토근 확인
    const token = req.headers.get("authorization")?.split(" ")[1];
    if (token === "null") return res(ctx.status(401));
  ...
  //2. 태그 자동완성
  rest.get("https://3.36.71.48/msw/hashtag", (req, res, ctx) => {
    //토근 확인
    const token = req.headers.get("authorization")?.split(" ")[1];
    if (token === "null") return res(ctx.status(401));
  ...
  //포스트 검색
  //1. 지금 인기 태그
  rest.get("https://3.36.71.48/msw/get_popular_tags", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
  ...
  //2. 포스트로 검색 (제목일치로 판단)
  rest.get("https://3.36.71.48/msw/search_post", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
  ...
  //3. 태그로 검색
  rest.get("https://3.36.71.48/msw/search_tag", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
  ...
  //4. 장소로 검색
  rest.get("https://3.36.71.48/msw/search_location", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
  ...
  //5. 유저로 검색
  rest.get("https://3.36.71.48/msw/search_username", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
  ...
  rest.get("https://3.36.71.48/msw/get_user_profile", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
    const username = req.url.searchParams.get("username");
  ...
  rest.get("https://3.36.71.48/msw/get_user_posts", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
    const username = req.url.searchParams.get("username");
    //username으로 유저 조회해서 결과 반환
  ...
  rest.get("https://3.36.71.48/msw/get_user_comments", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
    const username = req.url.searchParams.get("username");
    //username으로 유저 조회해서 결과 반환
    //res : [{포스트id, 원본이미지, 제목, 태그배열}]
  ...
  rest.get("https://3.36.71.48/msw/post/read", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
  ...
  //1-2. 포스트 댓글 정보 받기
  rest.get("https://3.36.71.48/msw/post/read/comments", (req, res, ctx) => {
    //토큰 검사 x (로그인 안해도 보여줘야 하기 때문)
    const post_id = req.url.searchParams.get("id");
    //res : {포스트 id, 작성자 유저네임, 작성날짜, 작성자 아이콘, 원본이미지, 감정:{crying:0, shock:0, laugh:20, thumb:1, heart:5}], 제목, 내용, 태그배열, 장소, 댓글배열, 작성자여부, 스크랩 여부(false, true), 등록한 이모지(설정하지 않았으면 null)}
    return res(
  ...
  rest.put<{ post_id: number; isScrap: boolean }>(
    "https://3.36.71.48/msw/post/scrap",
    (req, res, ctx) => {
      //토근 확인
      const token = req.headers.get("authorization")?.split(" ")[1];
      if (token === "null") return res(ctx.status(401));
  ...
  rest.put<{ post_id: number; myEmotion: string | null }>(
    "https://3.36.71.48/msw/post/emotion",
    (req, res, ctx) => {
      //토근 확인
      const token = req.headers.get("authorization")?.split(" ")[1];
      if (token === "null") return res(ctx.status(401));
  ...
  rest.post<{ post_id: number; comment: string; date: Date }>(
    "https://3.36.71.48/msw/post/comment/add",
    (req, res, ctx) => {
      //토근 확인
      const token = req.headers.get("authorization")?.split(" ")[1];
      if (token === "null") return res(ctx.status(401));
  ...
  rest.delete<{ post_id: number; comment_id: number }>(
    "https://3.36.71.48/msw/post/comment/delete",
    (req, res, ctx) => {
      //토근 확인
      const token = req.headers.get("authorization")?.split(" ")[1];
      if (token === "null") return res(ctx.status(401));
  ...
  rest.delete<{ post_id: number }>(
    "https://3.36.71.48/msw/post/delete",
    (req, res, ctx) => {
